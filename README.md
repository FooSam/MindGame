# 左右腦鍛鍊 (MindGame / Brain Training) 🧠✨

[![Kotlin](https://img.shields.io/badge/Kotlin-2.0+-7F52FF.svg?logo=kotlin&logoColor=white)](https://kotlinlang.org/)
[![Jetpack Compose](https://img.shields.io/badge/Jetpack%20Compose-Material%203-4285F4.svg?logo=android&logoColor=white)](https://developer.android.com/jetpack/compose)
[![Android SDK](https://img.shields.io/badge/Android%20SDK-24%20--%2036-3DDC84.svg?logo=android&logoColor=white)](https://developer.android.com/)
[![Room Database](https://img.shields.io/badge/SQLite-Room%20DB-FFA000.svg?logo=sqlite&logoColor=white)](https://developer.android.com/training/data-storage/room)
[![Google AdMob](https://img.shields.io/badge/Google%20AdMob-Integrated-E37400.svg?logo=googleadmob&logoColor=white)](https://admob.google.com/)
[![License](https://img.shields.io/badge/License-Proprietary-blue.svg)]()

> **「左右腦鍛鍊」**是一款專為全方位激發專注力、反應速度、邏輯推理、空間感知與情境思維所設計的現代化 Android 益智應用程式。全 UI 採用最新 **Jetpack Compose (Material 3)** 打造，支援極致流暢動效、豐富難易度、本地即時排行榜、中英雙語切換、7 套精緻視覺主題、無版權高品質背景音樂與智慧沉浸體驗。

---

## 🎮 核心遊戲訓練模組 (Game Modes)

| 遊戲項目 | 訓練領域 | 核心玩法特色 |
| :--- | :--- | :--- |
| 🧊 **六面合體 (3D Glass Puzzle Cube)** | **空間感知與幾何立體推理** | **旗艦級 3D 掌上型玻璃魔方拼圖！** 自由 360 度旋轉透明立方體，將 54 格立體水晶拼塊嚴密填滿 6 個面。具備面貼合磁吸預覽、托盤平滑滾動、清脆拼裝音效與極致晶瑩玻璃質感！ |
| 🍉 **水果切切樂 (Fruit Slicer)** | **反應速度與休閒解壓** | **暢快淋漓的休閒切水果解壓遊戲！** 滑動手指切碎漫天飛舞的新鮮水果，享受極致果汁飛濺爽感與連續 Combo 連擊；小心避開危險炸彈，提供普通與專家雙重挑戰模式！ |
| 🧱 **塊陶啊！陶藝方塊拼圖 (Escape Blocks)** | **空間感知與休閒解壓** | **首款「休閒遊戲」類別原創大作！免難度一鍵即玩！** 8×8 陶藝凹槽棋盤，自由拖放幾何陶土方塊。具備「滿行滿列變形為呆萌陶偶蜈蚣波浪竄逃」趣味動效、積累靈感條觸發「靈光一閃」方塊 90 度旋轉救場機制、雙向十字交叉爆破加分、連擊 Combo 水晶音階、陶瓷落子叩擊音效，以及 11 首放鬆音樂隨機輪播！ |
| 🐢 **海龜湯推理問答 (Turtle Soup)** | **情境推理與邏輯歸納** | **全量 100 道經典與原創情境推理謎題**！提供簡單 (Easy)、普通 (Medium)、困難 (Hard) 三種難度。創新 **4 大客觀維度（人物、物件、動機、場景）下拉組合問詢系統**，透過是非線索逐步揭開案件真相（湯底），支援星級評定與排行榜！ |
| 🔨 **打那個誰反應戰 (Avatar Whack)** | **反應打擊與抑制控制** | **21 位精緻 Q 版角色**！自動去背裁切，呈現露頭、假動作挑釁冒汗、被打到包紮金星 3 種生動表情。提供 2x2 到 3x4 六大階梯難度，具備假動作探頭（停留時間減半）、炸彈扣分障礙、黃金幸運星加分、連擊 Combo 火焰徽章與立體泥土洞口動效！ |
| 🧠 **斯特魯普效應 (Stroop Effect)** | **認知抗干擾與大腦彈性** | **看字說顏色經典認知挑戰**！提供 6 大難度進階模式：經典選字色、字義/顏色雙向切換、左右雙字比對（2選1快速判定）、文字與色彩雙重干擾按鈕、1.5 秒動態旋轉閃爍、以及複合形狀與否定句「選不是 X 色」史詩級搶答！ |
| 🎯 **專注力訓練 (Focus Train)** | **動態追蹤與視野辨識** | **創新動態舒爾特圓盤**！中心圓靜止，外圍 1~3 圈同心圓以順時針/逆時針動態旋轉（數字 1~69），文字維持正立向上，挑戰動態視覺專注力極限。 |
| ⏱️ **專注力測驗 (Focus Test)** | **視覺搜尋與抗干擾** | 經典舒爾特方格（Schulte Grid），提供 3x3 (初級 9格) 至 8x8 (史詩級 64格) 難度，依序點擊數字。 |
| ⚡ **極速配對 (Speed Match)** | **反應速度與瞬時記憶** | 1 分鐘限時挑戰！在隨機網格中找出唯一一組相同的數字組合，連續配對衝擊最高分。 |
| 🧩 **經典數獨 (Classic Sudoku)** | **邏輯思維與矩陣推演** | 提供 4x4 (入門)、6x6 (進階)、9x9 (標準三級難度) 與 12x12 (史詩級) 棋盤，支援筆記模式、擦除、復原與衝突高亮。 |
| 🐱 **貓咪數獨 (Cat Sudoku)** | **空間感知與幾何推理** | 創新色塊排貓益智玩法！每行、每列、每個同色區塊各恰好放置 1 隻貓咪，且相鄰 8 格不得有其他貓咪。 |

---

## 🧭 探索性引導與次選單排版革新 (Discovery & Navigation)

- 🗂️ **次選單手風琴可收合卡片 (Accordion Expandable Cards)**：
  - 各遊戲分類（智力遊戲、休閒遊戲、專注評估、邏輯推理）之次選單全面導入手風琴式收合卡片。
  - 卡片未展開時呈現精緻遊戲圖示、中英標題、專屬標籤與遊玩亮點摘要；點擊展開後直接提供「開始遊戲」與「🏆 排行榜」快捷鍵，大幅縮短玩家操作層級，直覺順手！
- 🌟 **首頁新遊戲探索橫幅 (Featured Spotlight Banner)**：
  - 首頁頂部智慧推薦旗艦新作（如《六面合體》、《水果切切樂》），支援一鍵直達開局，引導玩家快速體驗最新遊戲。
- ✨ **NEW 微脈動發光標籤 (New Badge)**：
  - 在新上線遊戲卡片與對應分類按鈕上，採用主題色彩之漸層微脈動呼吸燈動效標記，引導玩家輕鬆發掘全新內容。

---

## 🎵 豐富音樂與音效系統 (Audio Engine)

- 🎶 **11 首高品質無版權輕音樂**：收錄陽光放鬆、輕快愜意、清新舒暢、寧靜神秘、緊湊思維、自然療癒、俏皮歡樂、Lofi Study、陶藝禪風、寧靜鋼琴與陶笛微風等 11 首精選音樂，具備平滑淡入淡出循環。
- 🔀 **智慧切換演算法**：切換遊戲模式自動無縫換曲，單曲播放完畢隨機下一首（防重複機制）。
- 🎛️ **生命週期焦點管理**：切換至後台或手機鎖定時立即靜音暫停，返回 APP 即時復播。
- 🔊 **自主 PCM 音效合成與陶藝敲擊音效**：按鍵互動音效採用 Android AudioTrack 弦波演算法即時生成；專為《塊陶啊！》設計陶土落子敲擊音、水晶階梯連擊音、蜈蚣竄逃滑音與十字爆破音。

---

## 🎨 7 大主題風格與雙語支援 (Themes & Localization)

- ❄️ **雪白風格 (Snow White)** (預設明亮清爽)
- 🌙 **暗黑風格 (Dark Mode)** (極客沉浸感，省電舒適)
- ⚙️ **機械風格 (Mechanical)** (科技硬派金屬感)
- 🌸 **可愛風格 (Cute Pastel)** (溫馨柔和馬卡龍配色)
- ☀️ **陽光風格 (Sunny Vibrant)** (活力飽滿橙黃色調)
- 💼 **上班族風格 (Corporate)** (優雅專業商務藍調)
- ☕ **休閒風格 (Casual Earth)** (溫暖自然森系大地風)
- 🌐 **完整多語系**：繁體中文 (Traditional Chinese)、English。
- 🖥️ **全螢幕沉浸模式 (Immersive Sticky Mode)**：支援隱藏系統狀態列與虛擬導航列，享受無邊界全屏視野。

---

## 🏆 全球排行榜與玩家最愛應援投票 (Global Leaderboard & Favorite Voting)

- 🌐 **雙分頁排行榜架構 (Leaderboard Dialog)**：
  - **【🎮 單項紀錄榜】**：
    - 支援「遊戲切換選單（Game Selector Dropdown）」：在排行榜視窗內可隨意無縫切換專案中全部 11 款遊戲，支援各難度切換與本機/全球前 100 名高分即時刷新。
    - 個人最佳名次獨立高亮卡片：若不在前 100 名內，專屬卡片標示玩家精確全球排名與總參賽人數。
  - **【🔥 全球玩家熱門遊戲排行榜 (Hot Games)】**：
    - 展示當月全球所有玩家票選的最愛遊戲熱門度排行。
    - 呈現總參賽投票人數、各遊戲得票數、百分比人氣進度條。
    - 前三名尊爵金、銀、銅皇冠徽章與特殊漸層卡片；卡片右側附帶「🚀 立即暢玩」直達按鈕，達成一鍵玩熱門的完美體驗。
- ❤️ **玩家最愛遊戲應援投票系統 (Favorite Game Voting)**：
  - **首頁玩家名片應援條**：當月未投票時顯示發光脈動「前往投票 🔴」；投票完成後安靜展示 3 款最愛遊戲並提供隨時修改按鈕；跨月自動重置新賽季。
  - **最愛 3 款挑選彈窗 (`FavoriteGameVoteDialog`)**：卡片直覺點選，標記 ①、②、③ 序號，選滿 3 款一鍵送出至雲端。
  - **首頁精緻雙圖示按鈕組**：首頁右上角 42dp 對稱雙按鈕（🏆 全球風雲榜 + ⚙ 系統設定），適配各種手機螢幕寬度，未投票時掛載微脈動紅點引導。
- 🔄 **玩家資訊即時同步機制**：
  - 玩家修改名稱或更換國家/地區後，點擊「上傳至全球排行榜」，即使成績相同未破紀錄，**系統亦會以最後更新的資料為準，立即同步最新名稱、國旗與更新時間**至排行榜，同時維護歷史最佳戰績。
  - 每一位玩家在各遊戲難度維持單一最佳紀錄，杜絕重複刷榜或惡意霸榜行為。
- 🛡️ **私有雲端微後端與隱私保護架構**：
  - 本專案支援 Google Apps Script (GAS) 搭配 Google 試算表作為全球排行榜雲端後端。
  - **私密 URL 隔離原則**：為保障隱私與金鑰安全，正式私有 Google Drive 試算表及 GAS 部署網址**絕不公開於 Git 儲存庫**。
  - **本地配置說明**：正式端點統一由本機被 `.gitignore` 忽略之 `MindGame/local.properties`（或 CI/CD 環境變數 `GLOBAL_LEADERBOARD_URL`）注入：
    ```properties
    GLOBAL_LEADERBOARD_URL=https://script.google.com/macros/s/YOUR_SCRIPT_ID/exec
    ```
  - 若未設定 `GLOBAL_LEADERBOARD_URL`，系統將自動無縫啟用本地離線快取池與模擬排行榜，確保無網路或開源者本機編譯測試均能 100% 正常運行。

---

## 🛠️ 技術架構與技術棧 (Tech Stack)

- **開發語言**: Kotlin 2.0+
- **UI 框架**: Jetpack Compose (Material 3) + Canvas 極座標動態旋轉幾何運算
- **狀態管理**: Kotlin Coroutines & StateFlow (響應式單向數據流架構)
- **本地資料庫**: Android Room (SQLite)，離線儲存各遊戲模組排行榜成績
- **全球排行榜後端**: Google Apps Script (GAS) RESTful API 微後端 + 本地快取雙軌容錯架構
- **資料解析**: Moshi JSON 序列化引擎
- **廣告系統**: Google Mobile Ads SDK (AdMob 23.6.0)，具備每 3 局插頁廣告與次數獎勵補給機制
- **安全加固**: ProGuard / R8 程式碼混淆與無用資源壓縮
- **單元測試**: Robolectric, JUnit4, Roborazzi


---

## 🚀 本地編譯與建置 (Build & Run)

### 環境需求
- **JDK**: OpenJDK 17
- **Gradle**: 9.3.1+
- **Android SDK**: API 36 (minSdk 24, targetSdk 36)

### 編譯指令
```bash
# 執行所有自動化單元測試
gradle test

# 編譯 Debug APK
gradle :app:assembleDebug

# 正式簽署與 R8 加固 Release APK 打包
gradle :app:assembleRelease

# 正式產出 Google Play 發布專用 Android App Bundle (.aab)
gradle :app:bundleRelease
```

---

## 🔒 隱私與資料安全 (Privacy & Data Safety)
- **個人資料安全保護**：本應用程式遊戲進度與成績完全儲存於本地設備，不主動收集個人身分資料。
- **第三方廣告服務**：整合 Google AdMob 提供廣告服務，遵循 Google Play 廣告 ID 合規政策。
- **隱私權政策完整網址**：請參閱 [PRIVACY_POLICY.md](PRIVACY_POLICY.md) 或線上網頁 `https://FooSam.github.io/MindGame/privacy/`。

---

## 📄 版權聲明 (License)
Copyright &copy; 2026 **Ordinary People Studio**. All rights reserved.
