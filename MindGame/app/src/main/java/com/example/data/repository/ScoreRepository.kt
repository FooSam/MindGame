...
class ScoreRepository(private val scoreDao: ScoreDao) {
    fun getTopScores(categoryKey: String, gameTypeKey: String, difficultyKey: String, limit: Int = 10): Flow<List<ScoreRecord>> {
        return when (gameTypeKey) {
            GameType.SPEED_MATCH.key, GameType.TURTLE_SOUP.key, GameType.AVATAR_WHACK.key, GameType.STROOP_EFFECT.key -> scoreDao.getTopScoresByScore(categoryKey, gameTypeKey, difficultyKey, limit)
