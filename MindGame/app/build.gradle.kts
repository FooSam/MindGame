...
val (appVersionName, appVersionCode) = if (versionFile.exists()) {
  try {
    ...
    val verStr = json["APP_Version"] as? String ?: "1.00.00.01"
    val parts = verStr.split(".").map { it.toIntOrNull() ?: 0 }
    val code = if (parts.size == 4) parts[0] * 1000000 + parts[1] * 10000 + parts[2] * 100 + parts[3] else 1
    verStr to code
  } catch (e: Exception) {
    "1.00.00.01" to 1
  }
...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.mindgame.app"
    minSdk = 24
    targetSdk = 36
